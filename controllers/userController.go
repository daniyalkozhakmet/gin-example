...
func SignUp(c *gin.Context) {
	var body models.User
		// ShouldBindJSON binds the request body to the struct and performs validation
		if err := c.ShouldBindJSON(&body); err != nil {
			// Handle validation error and construct a custom response
			if ve, ok := err.(validator.ValidationErrors); ok {
				c.JSON(http.StatusBadRequest, gin.H{"error": utils.CustomErrorMessage(ve)})
				return
			}
